var W = 640, H = 1E3, IS_TOUCH, SCREEN_SHOW_ALL = !1;
(function (a, d) {
	function b() {
		...
		if (SCREEN_SHOW_ALL)
			k / b > W / H ? k = W * b / H : b = H * k / W, c.style.marginTop = 0;
		else {
			var d = W * b / H;
		...
		if (!h.noshare) {
			if (null == f) {
				...
				f.on("click", function (c) {
					IS_TOUCH &&
					c.nativeEvent instanceof MouseEvent || a.stage.removeChild(f)
				})
			}
			a.stage.addChild(f)
		}
	...
	a.showFollowAnim = function (c) {
		if (!h.followed) {
			if (null == g) {
				...
				g.name = "follow";
				g.on("click", function () {
					IS_TOUCH && event.nativeEvent instanceof MouseEvent || window.open(APP_FOLLOW_URL)
				})
			} else
	...
	a.showMoreGames = function (b) {
		if (null == e) {
			...
			e.hitArea = d;
			e.on("click", function () {
				IS_TOUCH && event.nativeEvent instanceof MouseEvent || window.open(APP_LIST_URL)
			});
			e.name = "moreGames"
		} else
			a.stage.removeChild(e);
		a.stage.addChild(e);
		e.visible = b
	}
...
function MyProgressBar(a, d, b) {
	...
	b.textAlign = "center";
	this.addChild(b);
	b = new createjs.Text("猫猫瑜伽", "bold 38px Arial", "black");
	b.x = a / 2;
	b.y = 240;
	b.textAlign = "center";
	this.addChild(b);
	b = new createjs.Text("yx8.com", "30px Arial", "black");
	b.x = a / 2;
	b.y = d - 80;
	b.textAlign = "center";
	this.addChild(b)
